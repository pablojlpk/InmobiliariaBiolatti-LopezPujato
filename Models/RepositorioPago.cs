...
namespace InmobiliariaBiolatti_LopezPujato.Models
{
    public class RepositorioPago
    {
        ...
        public IList<Pago> GetPagos()
        {
            ...
            {
                var sql = $"SELECT {nameof(Pago.idpago)}, {nameof(Pago.idcontrato)}, {nameof(Pago.importe)}, {nameof(Pago.fpago)} FROM pago WHERE borrado = 0 ORDER BY fpago ASC";
          ...
          public Pago GetPago(int id)
        {
            ...
            using (var connection = new MySqlConnection(ConnectionString))
            {
                var sql = $"SELECT {nameof(Pago.idpago)}, {nameof(Pago.idcontrato)}, {nameof(Pago.importe)}, {nameof(Pago.fpago)} FROM pago WHERE idpago = @idpago";
        ...
        public int Baja(int idpago)
        {
            ...
            using (var connection = new MySqlConnection(ConnectionString))
            {
                var sql = $"DELETE FROM pago WHERE {nameof(Pago.idpago)} = @idpago";
                using (var command = new MySqlCommand(sql, connection))
                {
        ...
        public int altaPago(Pago p)
        {
            ...
            using (var connection = new MySqlConnection(ConnectionString))
            {
                var sql = $"INSERT INTO pago (idcontrato, Monto, Afecha) VALUES (@idcontrato, @importe, @fpago)";
                using (var command = new MySqlCommand(sql, connection))
                {
        ...
        public int ModificaPago(Pago p)
        {
            ...
            using (var connection = new MySqlConnection(ConnectionString))
            {
                var sql = $"UPDATE pago SET idcontrato = @idcontrato, Monto = @importe, Afecha = @fpago WHERE {nameof(Pago.idpago)} = @idpago";
                using (var command = new MySqlCommand(sql, connection))
                {
